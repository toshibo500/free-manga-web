...
// カテゴリー別のマンガ一覧を取得する関数
export async function fetchMangasByCategory(
  categoryId: string,
): Promise<MangaListResponse> {
  try {
    console.log(
      `APIクライアント: カテゴリー別マンガ一覧取得 (カテゴリー: ${categoryId})`,
    );

    // Swagger 2.0のパスパターンに合わせる
    const response = await apiClient.get<MangaListResponse>(
    ...
    handleApiError(
      error,
      `カテゴリー別マンガ一覧の取得に失敗しました (カテゴリー: ${categoryId})`,
    );
    // エラーハンドラーが例外を投げるので、この行は実行されません
    throw new Error(
      `カテゴリー別マンガ一覧の取得に失敗しました (カテゴリー: ${categoryId})`,
